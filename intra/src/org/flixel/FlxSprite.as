package org.flixel
{
	...
	public class FlxSprite extends FlxObject
	{
		...
		/**
		* Positive x moves graphic left. Positive y moves graphic up. 
		*/
		...
		// stuff i added
		/**
		 * The current "pushdown" of the drawn frame. Used for example, making something rise out of the ground.
		 */
		public var framePixels_y_push:int = 0;
		
		public var gx:int = 0;
		public var gy:int = 0;
	
		/** 
		 * Whether or not this sprite should have tile callbacks. 
		 */
		
		public var has_tile_callbacks:Boolean = true;
		...
		/**
		 * Used in EventScripts::rotate_...
		 */
		public var rotate_angle:Number = 0;
		
		public var parabola_thing:Parabola_Thing;
		public var my_shadow:FlxSprite;
		public var sfx:Object = { };
		
		public const HURT_SOUND_NAME:String = "generic_hurt";
		...
		public function FlxSprite(X:Number=0,Y:Number=0,SimpleGraphic:Class=null)
		{
			...
			add_sfx(HURT_SOUND_NAME, Registry.sound_data.broom_hit);
		}
		...
		override public function destroy():void
		{
			...
			_callback = null;
			framePixels = null;
			if (parabola_thing != null) {
				parabola_thing.destroy();
				parabola_thing = null;
			}
			if (my_shadow != null) {
				my_shadow.destroy();
		...
		public function play(AnimName:String,Force:Boolean=false):void
		{
			...
			_frameTimer = 0;
			var i:uint = 0;
			// TODO this is null as dust playing unpoof?
			var l:uint = _animations.length;
			while(i < l)
			{
		...
		protected function calcFrame():void
		{
			...
			if (framePixels_y_push != 0) {
				/* blank the current frame */
				for (var i:int = 0; i < _flashRect.height; i++) {
					for (var j:int = 0; j < _flashRect.width; j++) {
						framePixels.setPixel32(j, i, 0x00000000);
					}
				}
				if (framePixels_y_push > 0) {
					//Move the area to be copied
					_flashRect.height  -= framePixels_y_push;
					_flashPointZero.y += framePixels_y_push;
					framePixels.copyPixels(_pixels, _flashRect, _flashPointZero);
					//Make that area normal again
					_flashRect.height += framePixels_y_push;
					_flashPointZero.y -= framePixels_y_push;
				}
			} else {
		...
		/**
		 * Add a sound effect to this FlxSprite
		 * @param	name	The name of the sound effect
		 * @param	sound_or_sound_group	The reference to the sound or sound group
		 * @return true on success, false on failure
		 */
		public function add_sfx(name:String, sound_or_sound_group:*):Boolean {
			if (FlxU.getClassName(sound_or_sound_group, true) == "FlxGroup") {
				sfx[name] = { is_group: true, sound: sound_or_sound_group };
			} else  if (FlxU.getClassName(sound_or_sound_group, true) == "FlxSound") {
				sfx[name] = { is_group: false, sound: sound_or_sound_group };
			} else {
				return false;
			}
			return true;
		...
		public function play_sfx(name:String,is_multi:Boolean=false):Boolean {
			if (!sfx.hasOwnProperty(name)) {
				FlxG.log("No such SFX \"" + name + "\"");
				return false;
			}
			
			var sound:* = sfx[name].sound;
			
			if (sfx[name].is_group) {
				if (is_multi) {
					Registry.sound_data.play_sound_group_randomly(sound);
				} else {
					Registry.sound_data.play_sound_group(sound);
				}
			} else {
				sound.play();
			}
		...
		public function stop_sfx(name:String):Boolean {
			if (!sfx.hasOwnProperty(name)) {
				FlxG.log("No such SFX \"" + name + "\"");
				return false;
			}
