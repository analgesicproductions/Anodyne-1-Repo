package states 
{
	...
	public class PauseState extends PushableFlxState
	{
		...
		{
			if (Registry.GAMESTATE != null && Registry.GAMESTATE.dialogue_state != null	) {
				Registry.GAMESTATE.dialogue_state.set_dialogue_box();
			}
			
			Registry.nr_growths = 0;
			for each (var val:int in Registry.card_states) {
				if (val == 1) {
					Registry.nr_growths += 1;
				}
			}
			
			text_categories.dirty = true;
			...
			text_info.text = Registry.controls[Keys.IDX_ACTION_1] + DH.lk("secrets",13)+" " + Registry.controls[Keys.IDX_ACTION_2] + DH.lk("secrets",14);
		...
		override public function create():void {
			...
			set_pausemenu_labels();
			
			secretz_wordz = new Array();
			for (var ij:int = 0; ij < 13; ij++) {
				secretz_wordz.push(DH.lk("secrets", ij));
			}
		...
		　	
			...
			playtime_text = EventScripts.init_bitmap_font("00:00:00", "left", 1, 156, null, "apple_white_OLD");
		...
		override public function update():void {
			...
			if (ctr_settings != 1 && ctr_settings != 5) {
				if (state == s_browse) {
			...
			if (!current_substate_visible) {
				...
				switch (base_index) {
					case 0: // Minimap
						...
						if (Registry.GE_States[Registry.GE_ENTERED_NEXUS_ONCE] == true && false == is_crowd_minidungeon()) {
							if (false == Registry.is_dungeon(Registry.CURRENT_MAP_NAME)) {
									...
									text_2.text = DH.lk("map", 4);
									if (DH.isZH()) text_2.text = text_2.text.replace("\n", "");
								if (minimap.has_map) {
					...
					case 1: // eqip
						...
						var normal_string:String = Registry.inventory[Registry.IDX_BROOM] ? DH.lk("items",1) : "-"; // Normal
						var status_1:String = Registry.inventory[Registry.IDX_LENGTHEN] ? DH.lk("items",3) : "-"; //Extend
						var status_2:String = Registry.inventory[Registry.IDX_WIDEN] ? DH.lk("items",4) : "-"; // WIden
						var status_3:String = Registry.inventory[Registry.IDX_TRANSFORMER] ? DH.lk("items",2) : "-"; // Swap
						
						text_1.visible = true;
						text_1.setText(normal_string + "\n\n\n" + status_1 + "\n\n\n" + status_2 + "\n\n\n" + status_3, true, 0, 0, "left", true);
						if (DH.isZH()) text_1.setText(normal_string+"\n\n"+status_1+"\n\n"+status_2+"\n\n"+status_3, true, 0, 0, "left", true);
						text_1.x = equip_boxes.members[0].x + equip_boxes.members[0].width + 4;
						text_1.y = 30;
					...
					case 2: //card
						...
						if (DH.language_type == DH.LANG_ja) {
							text_2.setText(Registry.nr_growths.toString() +DH.lk("cards",1), true, 0, 0, "left", true);
						} else {
							text_2.setText(Registry.nr_growths.toString() + " "+DH.lk("cards",1), true, 0, 0, "left", true);
						}
						text_2.x = 70;
					...
					case 3: // save 
						text_1.visible = true;
						if (Intra.is_ios) {
							//text_1.setText("Save\nSave and go\n to title\nGo to title", true, 0, 0, "left", true)
							text_1.setText(DH.lk("save",0)+"\n"+DH.lk("save",3)+"\n"+DH.lk("save",4), true, 0, 0, "left", true)
						} else {
							//text_1.setText("Save\nSave and go\n to title\nSave and quit\nQuit game", true, 0, 0, "left", true);
							text_1.setText(DH.lk("save",0)+"\n"+DH.lk("save",3)+"\n"+DH.lk("save",5)+"\n"+DH.lk("save",6), true, 0, 0, "left", true);
						}
						//text_1.text += "\n\n\n\nDeaths: " + Registry.death_count.toString();
						text_1.text += "\n\n\n\n"+DH.lk("save",7)+" " + Registry.death_count.toString();
					...
					case 4: //settings
						...
						var autosave_state:String = Registry.autosave_on ? DH.lk("config",4) : DH.lk("config",5);
						...
						text_3.x = 68;
						text_3.y = 120;
						if (DH.isZH()) text_3.y = 125;
						if (Intra.is_mobile) text_3.visible = false;
						text_3.setText(DH.lk("config",16)+" " + Intra.scale_factor.toString() + "x",true,0,0,"left",true);
			...
			if (Registry.keywatch.JP_ACTION_1 || Registry.keywatch.JP_RIGHT) {
				...
				switch (base_index) {
					case 0:
						...
						if (DH.isZH()) 	selectors.members[1].y = 142;
					...
					case 2:
						...
						if (DH.isZH()) selectors.members[1].y += 4;
						if (DH.isZH()) selectors.members[2].y += 4;
						selectors.members[1].play("enabled");
		...
		private function save_logic():void {
			...
			var spacin:int = 8;
			if (DH.isZH()) spacin = 12;
			if (Registry.keywatch.JP_DOWN) {
				if (save_cursor_idx < 3) {
					if (Intra.is_ios && save_cursor_idx == 2) {
						return;
					}
					save_cursor_idx++;
					if (save_cursor_idx == 2) {
						selectors.members[1].y += spacin*2;
					} else {
						selectors.members[1].y += spacin;
					}
				}
			} else if (Registry.keywatch.JP_UP) {
			...
			if (Registry.keywatch.JP_ACTION_1) {
				...
				case 2:
					if (Intra.is_ios) {
						...
						Save.save();
						if (Intra.IS_WINDOWS && Main.joy != null) {
							Main.joy.kill();
						}
					//if (Intra.is_ios) {
						//Intra.intraintra.dispatchEvent(new Event(Event.DEACTIVATE,true));
					//} else {
		...
		private var entered_lang:String;
		private function settings_logic():void {
			...
			if (ctr_settings == 0) {
				...
				if (Registry.keywatch.JP_DOWN) {
					if (settings_cursor_idx < 6) {
						settings_cursor_idx ++;
						if (settings_cursor_idx == 2) {
							selectors.members[1].y += 24;
						} else if (settings_cursor_idx == 3) {
							selectors.members[1].y += 32;
						} else if (settings_cursor_idx == 4) {
							selectors.members[1].y += 16;
							if (DH.isZH()) selectors.members[1].y += 8;
							//if (Intra.is_ios) {
								//settings_cursor_idx = 3;
							//} else if (Intra.is_mobile) {
								//settings_cursor_idx = 6;
								//selectors.members[1].y += 48;
							//}
						} else if (settings_cursor_idx == 5) {
							selectors.members[1].y += 16;
						} else if (settings_cursor_idx == 6) {
							if (Intra.is_mobile) {
								settings_cursor_idx --;
							} else {
								selectors.members[1].y += 8;
							}
						} else if (settings_cursor_idx == 7) {
							settings_cursor_idx = 6;
						} else {
							selectors.members[1].y += 16;
						}
				...
				if (Registry.keywatch.JP_ACTION_1) {
					if (settings_cursor_idx == 0) { 
						...
						if (!Intra.is_mobile) {
							...
							selectors.members[1].visible = false;
					volume_bar_bg.visible = false;
					volume_bar_overlay.visible = false;
					
							//text_1.text = "Drag the\nbuttons\nuntil you're\nsatisfied.\n\nThen, tap\nthe menu\nto continue.\n\n";
							text_1.text = DH.lk("config",20);
						...
						if (Intra.is_mobile) {
							// dpad + touch, dpad, touch
							var intra_ref:Intra = FlxG._game as Intra;
							if (Intra.allow_dpad_move && Intra.allow_game_move) {
								Intra.allow_dpad_move = true;
								Intra.allow_game_move = false;
							} else if (Intra.allow_dpad_move) {
								//intra_ref.turn_on_touch_only_listeners(true);
								Intra.allow_game_move = true;
								Intra.allow_dpad_move = false;
							} else if (Intra.allow_game_move) {
								//intra_ref.turn_on_touch_only_listeners(false);
								Intra.allow_dpad_move = true;
							}
							set_settings_text();
							} else {
						...
						selectors.members[2].visible = selectors.members[3].visible = true;
						selectors.members[2].scale.x = -1;
						selectors.members[2].x = selectors.members[1].x + 61; selectors.members[2].y = 136;
						selectors.members[3].x = 153; selectors.members[3].y = 136;
						
						if (DH.LANG_ja == DH.language_type || DH.language_type == DH.LANG_ko) {
							selectors.members[2].x -= 8 * 4;
							selectors.members[3].x -= 8 * 4;
						}
						if (DH.LANG_pt == DH.language_type || DH.language_type == DH.LANG_es) {
							selectors.members[2].x -= 8 * 2;
							selectors.members[3].x -= 8 * 2;
						}
						
					} else if (settings_cursor_idx == 6) {
						if (Intra.is_air && !Intra.is_mobile) {
							if (Intra.IS_WINDOWS && Main.joy.getTotal() > 0) {
								...
								ctr_settings = 5;
							} else if (Intra.IS_MAC && Main.mac_joy_manager != null && Main.mac_joy_manager.numberOfJoysticks() > 0 && Main.mac_joy_manager.joysticks[0] != null) {
								trace("JOYPAD DETECTED!");
								Keys.has_joypad = true;
								Keys.init_joypad();
				...
				if (Intra.is_mobile) {
					...
					if (MobileConfig.update()) {
						ctr_settings = 0;
						set_settings_text(false);
						volume_bar.visible = true;
						selectors.members[1].visible = true;
						volume_bar_bg.visible = true;
						volume_bar_overlay.visible = true;
					}
					return;
				}
				...
				if (Registry.keywatch.JP_LEFT) {
					...
					if (FlxG._game.stage.fullScreenHeight < ratio * 540*0.4 || FlxG._game.stage.fullScreenWidth < ratio * 480*0.4) {
			...
			} else if (ctr_settings == 5) { // Joypad
				if (Registry.joy_grp.is_done()) {
					remove(Registry.joy_grp, true);
					ctr_settings = 0;
				}
			} else if (ctr_settings == 24) { //language
				
				if (Registry.keywatch.JP_ACTION_1) {
					if (entered_lang != DH.language_type) {
						DH.set_language(DH.language_type);
						entered_lang = DH.language_type;
						Registry.keywatch.JP_ACTION_2 = true;
					}
				}
				if (Registry.keywatch.JP_ACTION_2) {
					if (DH.language_type != entered_lang) {
						DH.language_type = entered_lang;
					}
				...
				// [en,ja,ko,es,pt-br,it,zhs]
				if (Registry.keywatch.JP_RIGHT) {
					if (DH.language_type == DH.LANG_en) {
						DH.language_type = DH.LANG_ja;
					} else if (DH.language_type == DH.LANG_ja) {
						DH.language_type = DH.LANG_ko;
					} else if (DH.language_type == DH.LANG_ko) {
						DH.language_type = DH.LANG_es;
					} else if (DH.language_type == DH.LANG_es) {
						DH.language_type = DH.LANG_pt;
					}  else if (DH.language_type == DH.LANG_pt) {
						DH.language_type = DH.LANG_it;
					}  else if (DH.language_type == DH.LANG_it) {
						DH.language_type = DH.LANG_zhs;
					} else {
						DH.language_type = DH.LANG_en;
					}
					set_settings_text(false,entered_lang);
				} else if (Registry.keywatch.JP_LEFT) {
					if (DH.language_type == DH.LANG_ja) {
						DH.language_type = DH.LANG_en;
					} else if (DH.language_type == DH.LANG_en) {
						DH.language_type = DH.LANG_zhs;
			...
			if (text_1.text.length < 11) {
				...
				switch (text_1.text) {
					case " UUDDLRLR21":
						...
						for (var ij:int = 0; ij < 24; ij++) {
							Registry.inventory[ij] = true;
							Registry.card_states[ij] = 1;
						}
		...
		private function resize_mobile():void {
			switch (Intra.scale_ctr) {
				case Intra.SCALE_TYPE_FIT:
					Intra.scale_ctr = Intra.SCALE_TYPE_INT;
					break;
				case Intra.SCALE_TYPE_INT:
				default:
		...
		private function get_resolution_string(is_init:Boolean=false):String {
			...
			var res:String = DH.lk("config",13);
			switch (Intra.scale_ctr) {
				case 1:
					res = DH.lk("config",14);
					break;
		...
		private function set_settings_text(is_init:Boolean = false, enteredlang:String = ""):void {
			if (enteredlang == "") enteredlang = DH.language_type;
			var autosave_state:String = Registry.autosave_on ? DH.lk("config",4) : DH.lk("config",5); //on off
			var first:String = Intra.is_mobile ? DH.lk("config",7) : DH.lk("config",1); //config ui / set keybinds
			//var last:String = Intra.is_mobile ? " " : DH.lk("config",21); // " " / resize window
			var last:String = Intra.is_mobile ? " " : " "; // " " / resize window
			var res:String = Intra.is_mobile ? "\n"+DH.lk("config",6)+"\n " : "\n\n"+DH.lk("config",12)+"\n "; // change res/ resolution
			var joypart:String = Intra.is_ios ? " " : DH.lk("config",22);
			var langpart:String  = DH.lk("config", 17) +" " + String(DH.language_type);
			if (DH.LANG_pt == DH.language_type) {
				langpart = DH.lk("config", 17) +" " + "pt-BR";
			}
			if (entered_lang == DH.LANG_zhs) {
				langpart = DH.lk("config", 17)  + String(DH.language_type);
				langpart = langpart.replace(" ", "");
			}
			
			if (enteredlang == DH.LANG_ja) {
			...
			if (Intra.allow_dpad_move && Intra.allow_game_move) {
				//movepart = "Touch+D-Pad";
				movepart = DH.lk("config",8);
			} else if (Intra.allow_dpad_move) {
				movepart = DH.lk("config",9);
			} else if (Intra.allow_game_move) {
				movepart = DH.lk("config",10);
			}
			
			if (Intra.is_mobile) {
				text_1.setText(first + "\n\n" + DH.lk("config", 2) + "\n\n\n" + DH.lk("config", 3) + "\n    " + autosave_state + res + get_resolution_string(is_init) + "\n" + DH.lk("config", 11) + "\n" + movepart + "\n" + langpart, true, 0, 0, "left", true);
				if (enteredlang==DH.LANG_zhs) text_1.setText(first + "\n" + DH.lk("config", 2) + "\n\n" + DH.lk("config", 3)  + autosave_state + res + get_resolution_string(is_init) + "\n" + DH.lk("config", 11) + "\n" + movepart+"\n"+ langpart, true, 0, 0, "left", true);
			} else {
				if (is_init) {
					text_1.setText(first + "\n\n" + DH.lk("config", 2) + "\n\n\n" + DH.lk("config", 3) + "\n    " + autosave_state + res + get_resolution_string(true) + "\n\n\n" + langpart + "\n" + joypart, true, 0, 0, "left", true);
		...
		// do this eventually
		public function set_pausemenu_labels():void 
		{
			var idx:int = -1;
			var an_string:String = DH.lk("map", 0) + "\n\n" + DH.lk("items", 0) + "\n\n" + DH.lk("cards", 0) + "\n\n" + DH.lk("save", 0) + "\n\n" + DH.lk("config", 0);
			if (DH.isZH()) an_string = DH.lk("map", 0) + "\n" + DH.lk("items", 0) + "\n" + DH.lk("cards", 0) + "\n" + DH.lk("save", 0) + "\n" + DH.lk("config", 0);
			if (Registry.GE_States[Registry.GE_HAVE_A_SECRET]) {
				if (DH.isZH()) {
					an_string += "\n???";
				} else {
			...
			if (text_categories != null) {
				idx = base_group.members.indexOf(text_categories);
			}
			text_categories = EventScripts.init_bitmap_font(an_string, "left", 10, 13 + Registry.HEADER_HEIGHT, null, "apple_white");
			if (DH.isZH()) {
				text_categories.y -= 3;
				text_categories.customSpacingY += 4;
			}
			text_categories.color = 0xffffff;
			text_categories.drop_shadow = true;
			if (idx != -1) {
				base_group.members[idx] = text_categories;
			}
			
			idx = -1;
			if (text_info != null) idx = base_group.members.indexOf(text_info);
			...
			text_info = EventScripts.init_bitmap_font(Registry.controls[Keys.IDX_ACTION_1] + DH.lk("secrets",13)+ Registry.controls[Keys.IDX_ACTION_2] + DH.lk("secrets",14), "left", 2, 150 + Registry.HEADER_HEIGHT, null, "apple_white");
			text_info.color = 0x909ab1;
			if (idx != -1) base_group.members[idx] = text_info;
			
			//11323b
			idx = -1;
			if (text_1 != null) idx = base_group.members.indexOf(text_1);
			text_1 = EventScripts.init_bitmap_font(" ", "left", 0, 0, null, "apple_white");
			if (idx != -1) base_group.members[idx] = text_1;
			
			idx = -1;
			if (text_2 != null) idx = base_group.members.indexOf(text_2);
			text_2 = EventScripts.init_bitmap_font(" ", "left",0,0,null,"apple_white");
			if (idx != -1) base_group.members[idx] = text_2;
			
			idx = -1;
			if (text_3 != null) idx = base_group.members.indexOf(text_3);
			text_3 = EventScripts.init_bitmap_font(" ", "left", 0, 0, null, "apple_white");
			if (idx != -1) base_group.members[idx] = text_3;
